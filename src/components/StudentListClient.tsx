...
import { deleteStudentEnrollment } from '@/app/actions';
  ...
  return (
    <div className="glass-container" style={{ padding: '1.5rem', marginTop: '1rem' }}>
      ...
      {enrollments.length === 0 ? (
        <p className="text-muted">ยังไม่มีรายชื่อนักศึกษาในรายวิชานี้ กรุณาอัปโหลดไฟล์ CSV</p>
          ...
          {filteredEnrollments.map((enrollment) => (
            ...
            >
              <div 
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.2)'; e.currentTarget.style.border = '1px solid var(--primary-color)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.border = '1px solid transparent'; }}
              >
                <div>
                  <div style={{ fontWeight: 'bold' }}>{enrollment.student.studentId}</div>
                  <div style={{ fontSize: '0.9rem', color: '#666' }}>{enrollment.student.name}</div>
                </div>
                <button
